...
class Graph:
    ...
    def check_edge_in_mst(self, p, q):
        n = len(self.vertices)
        ...
        while not queue.empty():
            ...
            for j in self.vertices[i]:
                if j in queue and costs[j] > self.vertices[i][j]:
                    costs[j] = self.vertices[i][j]
                    parent[j] = i
                    queue.updatePriority(j, costs[j])
...
def main():
    ...
    for _ in range(t):
        ...
        print(graph.check_edge_in_mst(p, q))
